...
public class GamePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Dimensions fixes du panneau d'infos
        int infoPanelWidth = 200;
        int infoPanelHeight = 350;
        int spacing = 30; // espace entre la grille et le panneau d'infos

        // Calcul dynamique de la taille des cases
        int cellSize = (int) Math.min(
            (getWidth() - infoPanelWidth - spacing*2) / (double) Constants.COLS,
            (getHeight() - 40) / (double) Constants.ROWS
        );
        int boardWidth = cellSize * Constants.COLS;
        int boardHeight = cellSize * Constants.ROWS;
        int totalWidth = boardWidth + spacing + infoPanelWidth;
        int totalHeight = Math.max(boardHeight, infoPanelHeight);

        // Centrage global
        int xOrigin = (getWidth() - totalWidth) / 2;
        int yOrigin = (getHeight() - totalHeight) / 2;

        // Offsets pour la grille et le panneau d'infos
        int xOffset = xOrigin;
        int yOffset = yOrigin + (totalHeight - boardHeight) / 2;
        int infoX = xOffset + boardWidth + spacing;
        int infoY = yOrigin + (totalHeight - infoPanelHeight) / 2;
        ...
        // --- DESSIN DU PANNEAU D'INFOS ---
        drawInfoPanel(g2d, infoX, infoY);
