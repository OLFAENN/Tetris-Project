...
public class GuestFrame extends JFrame {
    ...
    public GuestFrame() {
        ...
        setTitle("Mode Invité - Tetris Cosmos");
    ...
    private void setupUI() {
        ...
        glassPanel.setPreferredSize(new Dimension(500, 300));
        glassPanel.setLayout(new BoxLayout(glassPanel, BoxLayout.Y_AXIS));
        glassPanel.setBorder(BorderFactory.createEmptyBorder(40, 40, 40, 40));

        // Titre stylé
        JLabel titleLabel = new JLabel("Jouer en invité");
        ...
        usernameField = new PlaceholderTextField("Entrez votre nom d’invité");
        ...
        // Bouton Valider
        JButton validateButton = new StylishButton("C’est parti !");
        validateButton.setAlignmentX(Component.CENTER_ALIGNMENT);
        validateButton.setMaximumSize(new Dimension(220, 50));
        validateButton.setBorder(BorderFactory.createEmptyBorder(10, 40, 10, 40));
        validateButton.setCursor(new Cursor(Cursor.HAND_CURSOR));
        validateButton.setForeground(Color.WHITE);
        validateButton.addActionListener(e -> handleValidate());
    ...
    private void handleValidate() {
        ...
        if (username.isEmpty()) {
            showThemedError("Veuillez entrer un nom d'utilisateur.");
            return;
        }
        try {
            User guestUser = userDAO.createGuestUser(username);
            GameFrame gameFrame = new GameFrame(guestUser);
            gameFrame.setVisible(true);
            this.dispose();
        } catch (SQLException ex) {
            showThemedError("Erreur lors de la création du compte invité: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
    ...
    // --- PlaceholderTextField (identique à Login/Register) ---
    ...
    // --- StylishButton (identique à Login/Register) ---
    ...
    // --- ThemedDialog pour messages d'erreur ---
